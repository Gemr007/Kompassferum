...
В комплекте — мини-приложение для ученика и кабинет педагога: тёмная тема на
палитре MAX UI, без сборки и без фронтенд-фреймворков.
...
| HTTP | httpx (async) |
| Фронтенд | Статика без сборки: HTML + ванильный JS, палитра MAX UI |
...
| http://localhost/static/teacher.html | кабинет педагога: сводка по классу |
...
soft skills. Ответы лежат в `localStorage`, поэтому тест можно закрыть на любом
вопросе и продолжить с того же места. В конце — карточка лучшего совпадения,
...
- **Прогресс теста хранится в браузере**, а не на сервере: со сменой устройства
  незавершённый тест не переедет. Серверное хранение черновика — следующий шаг.
- **Кабинет педагога не защищён паролем** — достаточно знать `max_user_id`
  пользователя с ролью teacher.
